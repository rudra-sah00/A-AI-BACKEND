...
class OllamaVisionProcessor(BaseProcessor):
    """
    Processor for handling contextual questions using Ollama vision models
    """
    def __init__(self, camera_data: Dict, users_data: Dict, output_dir: str):
        ...
        self.model = "bakllava:7b-v1-q4_0"  # Default model
        self.ollama_url = "http://localhost:11434/api/generate"  # Default Ollama API URL
        ...
        logger.info(f"Initialized OllamaVision processor for camera {self.camera_data.get('name')}")
    ...
    def _load_prompts(self) -> Dict:
        ...
        try:
            ...
            if os.path.exists(prompts_file):
                ...
                return {
                    "system_context": "You are an advanced AI-powered Security Surveillance System with expertise in visual analysis and threat detection.",
                    "camera_prompts": {
                        "default": "Analyze this security camera feed and answer the user's question."
                    }
                }
    ...
    def process_query(self, query: str) -> Dict:
        """
        Process a contextual query using the Ollama vision model
        ...
        if not self.is_active:
            return {
                "success": False,
                "error": f"OllamaVision processor for camera {self.camera_data.get('id')} is not active. Please wait for system to initialize."
            }
        ...
        with self.query_lock:
            ...
            try:
                ...
                # Get the system context and camera prompt
                ...
                logger.info(f"Calling Ollama vision API with model {self.model}, image size: {len(image_bytes) // 1024}KB")
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"Ollama vision API returned successful response: {preview}")
